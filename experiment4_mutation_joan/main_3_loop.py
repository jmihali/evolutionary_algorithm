...
if __name__ == '__main__':
    ...
    for seed in range(total_exp):
        ...
        if best_fitness[-1] < -170:
            num_fit120 += 1
            num_fit150 += 1
            num_fit170 += 1
        elif best_fitness[-1] < -150:
            num_fit120 += 1
            num_fit150 += 1
        elif best_fitness[-1] < -120:
            num_fit120 += 1

        if best_fitness[-1] < best_best_fitness[-1]:
    ...
    print('Experiments with a score better than -120: %d/%d\n' % (num_fit120, total_exp))
    print('Experiments with a score better than -150: %d/%d\n' % (num_fit150, total_exp))
    print('Experiments with a score better than -170: %d/%d\n' % (num_fit170, total_exp))

    print('\nBest experimental result:\n')
    print('Best individual: ', best_best_ind)
    print('Best fitness:', best_best_fitness[-1])

    global_min = -np.power(2.808, DIMENSIONS) # attention: hard coded
    global_min_line = plt.plot([0, gen[-1]], [global_min, global_min], 'g--', label='Global minimum')

    avg_line = plt.plot(gen, best_avg_fitness, 'r--', label='Average fitness')
    best_line = plt.plot(gen, best_best_fitness, 'b', label='Best fitness')
    global_min_line = plt.plot([0, gen[-1]], [global_min, global_min], 'g--', label='Global minimum')
